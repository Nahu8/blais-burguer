document.addEventListener('DOMContentLoaded', () => {
    ...
    addToCartButtons.forEach((button) => {
        button.addEventListener('click', () => {
            ...
            const productPriceOriginal = parseFloat(button.getAttribute('data-precio-original'));
            const productPricePromo = parseFloat(button.getAttribute('data-precio-promo'));
            const productImage = productCard.querySelector('img').src;

            const product = {
                name: productName,
                originalPrice: productPriceOriginal,
                promoPrice: productPricePromo,
                image: productImage,
            };
    ...
    function updateCart() {
        ...
        cart.forEach((product, index) => {
            ...
            cartItem.innerHTML = `
                ...
                <span><s>$${product.originalPrice.toFixed(2)}</s> $${product.promoPrice.toFixed(2)}</span>
